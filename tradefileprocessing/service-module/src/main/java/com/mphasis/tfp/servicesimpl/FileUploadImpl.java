...
public class FileUploadImpl implements IFileUpload {
    ...
    // ADD THESE METHODS TO FileUploadImpl CLASS
    ...
    public Page<ErrorResponseDTO> getErrorLogsPaginated(
        ...
        log.info("Paginated error logs - Page: {}, Size: {}, FileId: {}, FileName: {}",
                page, size, fileId, fileName);
        ...
        Page<FailedTransaction> failedPage = failedTransactionRepository.findErrorsWithPagination(
                fileId, fileName, pageable);

        // Convert entities to DTOs
        List<ErrorResponseDTO> content = serviceMapper.toErrorResponseDTOList(failedPage.getContent());

        log.info("Found {} error logs (Page {}/{})", content.size(), page + 1, failedPage.getTotalPages());
    ...
    public Page<FileLoadMetaDataResponse> searchFilesPaginated(
        ...
        log.info("Paginated search - Page: {}, Size: {}, Sort: {} {}", page, size, sortBy, sortDirection);
        ...
        LocalDateTime startDateTime = uploadDateFrom != null ? uploadDateFrom.atStartOfDay() : null;
        LocalDateTime endDateTime = uploadDateTo != null ? uploadDateTo.atTime(LocalTime.MAX) : null;

        // Fetch paginated data
        Page<TransactionMetadata> metadataPage = metadataRepository.searchFilesWithPagination(
                fileId, fileName, startDateTime, endDateTime, status, pageable);

        // Convert to DTOs
        List<FileLoadMetaDataResponse> content = serviceMapper.toFileLoadMetaDataResponseList(
                metadataPage.getContent());

        log.info("Found {} files (Page {}/{})", content.size(), page + 1, metadataPage.getTotalPages());

        return new PageImpl<>(content, pageable, metadataPage.getTotalElements());
    }
